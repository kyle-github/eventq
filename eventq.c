...
void platform_thread_create(platform_thread *thread, thread_func_t func, void *ctx) {
...
uint64_t platform_now_us(void) {
    ...
    return ((High / platform_perf_freq) << 32) + ((Low + ((High % platform_perf_freq) << 32)) / platform_perf_freq);
}
    #if EC_PSN
        #include <stdlib.h>
typedef DWORD(WSAAPI *PPROCESS_SOCKET_NOTIFICATIONS_ROUTINE)(HANDLE completionPort, UINT32 registrationCount,
                                                             SOCK_NOTIFY_REGISTRATION *registrationInfos, UINT32 timeoutMs,
                                                             ULONG completionCount, OVERLAPPED_ENTRY *completionPortEntries,
...
void eventq_cleanup(eventq *queue) { CloseHandle(*queue); }
...
bool eventq_socket_receive_start(eventq *queue, platform_socket *sock) {
    ...
    do {
        ...
        result = WSARecvFrom(sock->fd, &sock->buf, 1, &bytesRecv, &flags, &sock->recv_addr, &sock->recv_addr_len,
                             (OVERLAPPED *)&sock->recv_sqe, NULL);
...
void eventq_socket_send_start(eventq *queue, platform_socket *sock, uint32_t length) {
    ...
    if(result == SOCKET_ERROR) {
        if(WSAGetLastError() != WSA_IO_PENDING) { printf("WSASend failed with error: %d\n", WSAGetLastError()); }
    }
}
...
void eventq_socket_receive_complete(eventq *queue, eventq_cqe *cqe) {
    ...
    do {
        ...
        if(result <= 0) {
            if(WSAGetLastError() != WSAEWOULDBLOCK) { printf("recvfrom failed, %d\n", WSAGetLastError()); }
            break;
        }
...
void eventq_socket_send_start(eventq *queue, platform_socket *sock, uint32_t length) {
    ...
    if(result == SOCKET_ERROR) { printf("send failed with error: %d\n", WSAGetLastError()); }
}
void eventq_socket_send_complete(eventq *queue, eventq_cqe *cqe) {}
void eventq_enqueue(eventq *queue, eventq_sqe *sqe, uint32_t type, void *user_data, uint32_t status) {
    memset(sqe, 0, sizeof(*sqe));
...
void eventq_cleanup(eventq *queue) { close(*queue); }
...
void eventq_socket_receive_complete(eventq *queue, eventq_cqe *cqe) {
    ...
    do {
        ...
        result = recvfrom(sock->fd, sock->buffer, sizeof(sock->buffer), 0, &sock->recv_addr, &sock->recv_addr_len);
        if(result <= 0) {
            if(errno != EAGAIN && errno != EWOULDBLOCK) { printf("recvfrom failed, %d\n", errno); }
            break;
        }
        printf("Receive complete, %d bytes\n", result);
    } while(true);
...
void eventq_socket_send_start(eventq *queue, platform_socket *sock, uint32_t length) {
    ...
    if(send(sock->fd, sock->buffer, length, 0) < 0) {
        if(errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            if(kevent(*queue, &event, 1, NULL, 0, NULL) < 0) { printf("kevent(send) failed, %d\n", errno); }
        } else {
...
void eventq_enqueue(eventq *queue, eventq_sqe *sqe, uint32_t type, void *user_data, uint32_t status) {
    ...
    if(0 != kevent(*queue, &event, 1, NULL, 0, NULL)) { printf("kevent enqueue failed\n"); }
}
uint32_t eventq_dequeue(eventq *queue, eventq_cqe *events, uint32_t count, uint32_t wait_time) {
...
void eventq_socket_send_start(eventq *queue, platform_socket *sock, uint32_t length) {
    ...
    if(send(sock->fd, sock->buffer, length, 0) < 0) {
        if(errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            if(0 != epoll_ctl(*queue, EPOLL_CTL_ADD, sock->fd, &event)) { printf("epoll_ctl (send) failed\n"); }
        } else {
            printf("send failed, %d\n", errno);
        }
    }
...
uint32_t eventq_dequeue(eventq *queue, eventq_cqe *events, uint32_t count, uint32_t wait_time) {
    ...
    int result;
    do { result = epoll_wait(*queue, events, count, timeout); } while((result == -1L) && (errno == EINTR));
    return (uint32_t)result;
}
void eventq_return(eventq *queue, uint32_t count) {}
...
bool eventq_initialize(eventq *queue) { return 0 == io_uring_queue_init(256, queue, 0); }
void eventq_cleanup(eventq *queue) { io_uring_queue_exit(queue); }
bool eventq_sqe_initialize(eventq *queue, eventq_sqe *sqe) {}
void eventq_sqe_cleanup(eventq *queue, eventq_sqe *sqe) {}
bool eventq_socket_create(eventq *queue, platform_socket *sock) {
...
void eventq_return(eventq *queue, uint32_t count) { io_uring_cq_advance(queue, count); }
...
void platform_process_event(eventq *queue, eventq_cqe *cqe) {
    switch(eventq_cqe_get_type(cqe)) {
        case PLATFORM_EVENT_TYPE_SOCKET_RECEIVE: eventq_socket_receive_complete(queue, cqe); break;
        case PLATFORM_EVENT_TYPE_SOCKET_SEND: eventq_socket_send_complete(queue, cqe); break;
        default: printf("Unexpected Event %u\n", eventq_cqe_get_type(cqe)); break;
    }
}
...
bool platform_socket_create_client(platform_socket *sock, eventq *queue) {
    ...
    for(uint32_t i = 0; i < 50; ++i) { eventq_socket_send_start(queue, sock, 1); }
    return true;
}
            ...
            if(eventq_cqe_get_type(&events[i]) < APP_EVENT_TYPE_START) {
                ...
                switch((APP_EVENT_TYPE)eventq_cqe_get_type(&events[i])) {
                    case APP_EVENT_TYPE_SHUTDOWN:
                        ...
                        running = false;
                        break;
                    case APP_EVENT_TYPE_ECHO: printf("Echo event received\n"); break;
                    case APP_EVENT_TYPE_START_TIMER: platform_set_timer_ms(eventq_cqe_get_status(&events[i])); break;
...
void eventq_start(app_state *state) {
...
void eventq_stop(app_state *state) {
